...
```
The K-S test for the normality assumption shows that the `p`-value is 0.2261. So we can not reject the non hypothes that the standardized residuals follow a univariate normal distribution.
...
Now we compare the absolute returns $abs(y_t)$ with the estimated volatilities $\exp(hat{x_t}/2)$.
...
QQ_plot(model_fit$standard_residuals)
```
The K-S test for the normality assumption shows that the `p`-value is 0.2173. So we can not reject the non hypothesis that the standardized residuals follow a univariate normal distribution.
